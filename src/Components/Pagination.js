                            ...
                            <>
                                ...
                                {(item.label <= sellers.last_page) ? (
                                    <>
                                        {(item.active) ? (
                                            <>
                                                <button className="w-8 h-8 shadow rounded-full border-1 border-logobarElementBG bg-logobarElementBG text-white hover:bg-logobarElementBG hover:border-logobarElementBG active:bg-logobarElementBG mx-2 focus:outline-none font-Poppins font-normal text-sm">{item.label}</button>
                                            </>
                                        ) : (
                                            <>
                                                <button onClick={() => setUpdate(item.url)} className="w-8 h-8 shadow rounded-full border-1 border-logobarElementBG text-black hover:text-white hover:bg-logobarElementBG mx-2 focus:outline-none"><span dangerouslySetInnerHTML={{ __html: item.label }} /></button>
                                            </>
                                        )}
                                    </>
                                ) : (
