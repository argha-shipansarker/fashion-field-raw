...
import { updateSelectedAttribute, updateSelectedBrand } from "../../../ReduxStore/StoreFilteringSystem/StoreFilter"

const StorePageSidebar = props => {

    const { categories } = props;

    const { brands, attributes, mainSelectedAttributeArray, selectedBrand } = useSelector(state => state.storeFilter)
    ...
    const handleSelectedBrand = brand => {
        dispatch(updateSelectedBrand(brand.slug))
    }
                ...
                <Scrollbars style={{ height: 280 }}>
                    {
                        brands?.map((item, index) => (
                            <p className={`font-DMSans text-sm mb-3.25 block ${selectedBrand == item.slug ? "text-logobarElementBG" : "text-sliderDescription"}`} onClick={() => handleSelectedBrand(item)}><span className={`cursor-pointer hover:text-logobarElementBG `} key={index}>{item.name}</span></p>
                        ))
                    }
                </Scrollbars>
            </div>
...
export default StorePageSidebar
